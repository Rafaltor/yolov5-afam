...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg,\
        resume, noval, nosave, workers, freeze, custom_metric_t = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights,\
        opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze, opt.custom_metric_t
    ...
    if opt.optimizer == 'Adam':
        optimizer = Adam(g[2], lr=hyp['lr0'], betas=(
            hyp['momentum'], 0.999))  # adjust beta1 to momentum
    elif opt.optimizer == 'AdamW':
        optimizer = AdamW(g[2], lr=hyp['lr0'], betas=(
            hyp['momentum'], 0.999))  # adjust beta1 to momentum
    else:
    ...
    if cuda and RANK != -1:
        if check_version(torch.__version__, '1.11.0'):
            model = DDP(model, device_ids=[
                        LOCAL_RANK], output_device=LOCAL_RANK, static_graph=True)
        else:
            model = DDP(model, device_ids=[
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is last:  # in the original yolo it was 'best' instead
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val.run(
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--custom-metric-t', type=float, default=0.2,
                        help='custom metrics\'s treshold of iou for counting a box as positive')
